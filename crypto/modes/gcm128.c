...
static void gcm_get_funcs(struct gcm_funcs_st *ctx)
{
    ...
    /*
     * LoongArch GHASH.  All variants share the Htable built by
     * gcm_init_4bit.  A single-block multiply is cheaper with the 4-bit
     * table than with SIMD (which has to build its lookup tables on every
     * call), so gmult always stays scalar and only ghash is vectorised.
     */
